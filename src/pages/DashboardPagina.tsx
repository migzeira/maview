...
const DashboardPagina = () => {
  ...
  const previewEffectOverlay: React.ReactNode = (() => {
    ...
    switch (d.bgEffect) {
      ...
      /* ─── Motion effects ─── */
      case "ocean-waves": return wrap(<>{[0,1,2].map(i => <div key={i} className="absolute w-[250%] left-[-75%]" style={{ height: `${50 + i * 20}px`, bottom: `${i * 12}px`, background: `linear-gradient(180deg, transparent, ${i % 2 === 0 ? a : a2}50)`, borderRadius: "45% 45% 0 0", animation: `mvp-sway ${6 + i * 2}s ease-in-out infinite`, animationDelay: `${i * -2}s` }} />)}</>);
      case "orbit-circles": return wrap(<>{[0,1,2,3].map(i => <div key={i} className="absolute rounded-full" style={{ width: 10, height: 10, left: "50%", top: "50%", background: i % 2 === 0 ? a : a2, opacity: 0.8, animation: `mvp-orbit ${6 + i * 3}s linear infinite`, ["--orbit-r" as string]: `${30 + i * 18}px` } as React.CSSProperties} />)}</>);
      case "ripple-rings": return wrap(<>{[0,1,2].map(i => <div key={i} className="absolute rounded-full border-2" style={{ width: 60, height: 60, left: "calc(50% - 30px)", top: "calc(50% - 30px)", borderColor: `${a}60`, animation: "mvp-ripple 4s ease-out infinite", animationDelay: `${i * 1.3}s` }} />)}</>);
      case "morph-blobs": return wrap(<>{[{x:30,y:25},{x:65,y:60}].map((p,i) => <div key={i} className="absolute" style={{ width: 100, height: 100, left: `${p.x}%`, top: `${p.y}%`, transform: "translate(-50%,-50%)", background: `radial-gradient(circle, ${i === 0 ? a : a2}50, transparent 60%)`, borderRadius: "60% 40% 30% 70% / 60% 30% 70% 40%", animation: "mvp-morph 10s ease-in-out infinite", animationDelay: `${i * -5}s`, filter: "blur(6px)" }} />)}</>);
      case "orbit-rings": return wrap(<>{[0,1].map(i => <div key={i} className="absolute rounded-full border-2" style={{ width: 100 + i * 50, height: 100 + i * 50, left: "50%", top: "50%", transform: "translate(-50%,-50%)", borderColor: `${i === 0 ? a : a2}40`, animation: `mvp-spin ${20 + i * 10}s linear infinite` }} />)}</>);
      case "neon-lines": return wrap(<>{[0,1,2,3].map(i => <div key={i} className="absolute" style={{ width: "120%", height: 2, left: "-10%", top: `${20 + i * 20}%`, background: `linear-gradient(90deg, transparent, ${i % 2 === 0 ? a : a2}80, transparent)`, animation: `mvp-sway ${5 + i}s ease-in-out infinite`, animationDelay: `${i * -1.2}s` }} />)}</>);
      case "gradient-shift-hue": return wrap(<div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${a}60, ${a2}55, ${a}60)`, animation: "mvp-hue 12s linear infinite" }} />);
      case "light-sweep": return wrap(<div className="absolute" style={{ width: "40%", height: "150%", top: "-25%", left: "-40%", background: `linear-gradient(90deg, transparent, ${a}45, transparent)`, transform: "skewX(-15deg)", animation: "mvp-sweep 4s ease-in-out infinite" }} />);
      case "breathing-glow": return wrap(<div className="absolute inset-0" style={{ background: `radial-gradient(circle at 50% 50%, ${a}50, transparent 65%)`, animation: "mvp-breathe 5s ease-in-out infinite" }} />);
      case "conic-spotlight": return wrap(<div className="absolute" style={{ width: 200, height: 200, left: "calc(50% - 100px)", top: "calc(50% - 100px)", background: `conic-gradient(from 0deg, ${a}55, transparent 30%, transparent 70%, ${a2}50)`, borderRadius: "50%", animation: "mvp-spin 20s linear infinite" }} />);
      case "rising-particles": return wrap(<>{[0,1,2,3,4].map(i => <div key={i} className="absolute rounded-full" style={{ width: 6, height: 6, left: `${15 + i * 18}%`, bottom: 0, background: i % 2 === 0 ? a : a2, opacity: 0.7, animation: `mvp-rise ${5 + i}s ease-in-out infinite`, animationDelay: `${i * -1}s` }} />)}</>);
      case "noise-flicker": return wrap(<div className="absolute inset-0" style={{ background: `repeating-conic-gradient(${a}18 0% 25%, transparent 0% 50%) 0 0 / 4px 4px`, animation: "mvp-flicker 3s steps(3) infinite" }} />);
      case "diagonal-shimmer": return wrap(<div className="absolute inset-0" style={{ background: `repeating-linear-gradient(45deg, transparent 0px, transparent 8px, ${a}25 8px, ${a}25 10px)`, backgroundSize: "200% 200%", animation: "mvp-slide-diag 10s linear infinite" }} />);
      case "floating-orbs": return wrap(<>{[{x:15,y:10,s:90},{x:70,y:50,s:70},{x:35,y:75,s:60}].map((p,i) => <div key={i} className="absolute rounded-full" style={{ width: p.s, height: p.s, left: `${p.x}%`, top: `${p.y}%`, background: `radial-gradient(circle, ${i % 2 === 0 ? a : a2}50, transparent 60%)`, filter: "blur(10px)", animation: `mvp-drift ${12 + i * 5}s ease-in-out infinite`, animationDelay: `${i * -4}s` }} />)}</>);
      case "layered-waves": return wrap(<>{[0,1,2,3].map(i => <div key={i} className="absolute w-[250%] left-[-75%]" style={{ height: `${40 + i * 15}px`, bottom: `${i * 12}px`, background: `linear-gradient(180deg, transparent, ${i % 2 === 0 ? a : a2}45)`, borderRadius: "45% 45% 0 0", animation: `mvp-sway ${5 + i * 1.5}s ease-in-out infinite`, animationDelay: `${i * -1.5}s` }} />)}</>);
      case "pulse-circles": return wrap(<>{[0,1,2].map(i => <div key={i} className="absolute rounded-full" style={{ width: 60, height: 60, left: "calc(50% - 30px)", top: "calc(50% - 30px)", border: `2px solid ${a}55`, animation: "mvp-ripple 5s ease-out infinite", animationDelay: `${i * 1.6}s` }} />)}</>);
      case "gradient-aurora-mesh": return wrap(<><div className="absolute w-[200%] h-[60%] top-[-10%] left-[-50%]" style={{ background: `linear-gradient(135deg, ${a}55, transparent 40%, ${a2}45, transparent)`, filter: "blur(18px)", animation: "mvp-aurora 10s ease-in-out infinite" }} /><div className="absolute w-[200%] h-[50%] bottom-[-10%] left-[-30%]" style={{ background: `linear-gradient(225deg, ${a2}45, transparent 40%, ${a}40, transparent)`, filter: "blur(18px)", animation: "mvp-aurora 14s ease-in-out infinite reverse" }} /></>);
      case "stripe-gradient": return wrap(<div className="absolute inset-0" style={{ background: `repeating-linear-gradient(135deg, ${a}30 0px, ${a}30 4px, transparent 4px, transparent 12px)`, animation: "mvp-slide-diag 12s linear infinite", backgroundSize: "200% 200%" }} />);
      case "vortex-spin": return wrap(<div className="absolute" style={{ width: 200, height: 200, left: "calc(50% - 100px)", top: "calc(50% - 100px)", background: `conic-gradient(from 0deg, ${a}50, ${a2}40, transparent, ${a}50)`, borderRadius: "50%", filter: "blur(10px)", animation: "mvp-spin 15s linear infinite" }} />);
      case "liquid-glass": return wrap(<>{[{x:25,y:30},{x:65,y:55}].map((p,i) => <div key={i} className="absolute" style={{ width: 110, height: 110, left: `${p.x}%`, top: `${p.y}%`, transform: "translate(-50%,-50%)", background: `radial-gradient(circle, ${i === 0 ? a : a2}40, transparent 60%)`, borderRadius: "60% 40% 30% 70% / 60% 30% 70% 40%", animation: "mvp-morph 8s ease-in-out infinite", animationDelay: `${i * -4}s`, filter: "blur(8px)" }} />)}</>);
      default: return null;
    }
  })();
