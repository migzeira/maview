...
const DashboardPagina = () => {
  ...
  const previewEffectOverlay: React.ReactNode = (() => {
    ...
    switch (d.bgEffect) {
      ...
      case "ocean-waves":
      case "layered-waves":
        return wrap(<>{[0,1,2,3,4].map(i => <div key={i} className="absolute w-[300%] left-[-100%]" style={{ height: `${60 + i * 22}px`, bottom: `${i * 18}px`, background: `linear-gradient(180deg, transparent 15%, ${i % 2 === 0 ? a : a2}${i < 2 ? "90" : "60"})`, borderRadius: "42% 58% 42% 58% / 100% 100% 0% 0%", animation: `mvp-wave-ud ${3 + i * 0.8}s ease-in-out infinite`, animationDelay: `${i * -0.7}s` }} />)}</>);
      ...
      case "vortex-spin": return wrap(<div className="absolute" style={{ width: 280, height: 280, left: "calc(50% - 140px)", top: "calc(55% - 140px)", background: `conic-gradient(from 0deg, ${a}70, ${a2}50, transparent 35%, transparent 55%, ${a}60, ${a2}40, transparent 85%)`, borderRadius: "50%", filter: "blur(8px)", animation: "mvp-spin 8s linear infinite" }} />);
