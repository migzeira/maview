...
const DashboardPagina = () => {
  ...
  const previewEffectOverlay: React.ReactNode = (() => {
    ...
    switch (d.bgEffect) {
      ...
      /* ─── Motion effects (large, high-contrast for mini preview) ─── */
      case "ocean-waves": return wrap(<>{[0,1,2,3].map(i => <div key={i} className="absolute w-[300%] left-[-100%]" style={{ height: `${70 + i * 25}px`, bottom: `${i * 20}px`, background: `linear-gradient(180deg, transparent 20%, ${i % 2 === 0 ? a : a2}90)`, borderRadius: "45% 45% 0 0", animation: `mvp-sway ${5 + i}s ease-in-out infinite`, animationDelay: `${i * -1.5}s` }} />)}</>);
      case "orbit-circles": return wrap(<>{[0,1,2,3,4].map(i => <div key={i} className="absolute rounded-full" style={{ width: 12 - i, height: 12 - i, left: "50%", top: "45%", background: i % 2 === 0 ? a : a2, opacity: 1, boxShadow: `0 0 8px ${i % 2 === 0 ? a : a2}`, animation: `mvp-orbit ${5 + i * 2}s linear infinite`, ["--orbit-r" as string]: `${25 + i * 20}px` } as React.CSSProperties} />)}</>);
      case "ripple-rings": return wrap(<>{[0,1,2,3].map(i => <div key={i} className="absolute rounded-full" style={{ width: 80, height: 80, left: "calc(50% - 40px)", top: "calc(45% - 40px)", border: `2px solid ${a}`, opacity: 0.7, animation: "mvp-ripple 3.5s ease-out infinite", animationDelay: `${i * 0.9}s` }} />)}</>);
      case "morph-blobs": return wrap(<>{[{x:30,y:20,s:160},{x:70,y:65,s:140}].map((p,i) => <div key={i} className="absolute" style={{ width: p.s, height: p.s, left: `${p.x}%`, top: `${p.y}%`, transform: "translate(-50%,-50%)", background: `radial-gradient(circle, ${i === 0 ? a : a2}70, transparent 70%)`, borderRadius: "60% 40% 30% 70% / 60% 30% 70% 40%", animation: "mvp-morph 8s ease-in-out infinite", animationDelay: `${i * -4}s`, filter: "blur(4px)" }} />)}</>);
      case "orbit-rings": return wrap(<>{[0,1,2].map(i => <div key={i} className="absolute rounded-full" style={{ width: 80 + i * 60, height: 80 + i * 60, left: "50%", top: "45%", transform: "translate(-50%,-50%)", border: `2px solid ${i % 2 === 0 ? a : a2}`, opacity: 0.5, animation: `mvp-spin ${15 + i * 8}s linear infinite`, animationDirection: i % 2 === 0 ? "normal" : "reverse" }} />)}</>);
      case "neon-lines": return wrap(<>{[0,1,2,3,4].map(i => <div key={i} className="absolute" style={{ width: "130%", height: 3, left: "-15%", top: `${15 + i * 18}%`, background: `linear-gradient(90deg, transparent 5%, ${i % 2 === 0 ? a : a2} 50%, transparent 95%)`, opacity: 0.7, boxShadow: `0 0 6px ${i % 2 === 0 ? a : a2}60`, animation: `mvp-sway ${4 + i}s ease-in-out infinite`, animationDelay: `${i * -1}s` }} />)}</>);
      ...
      case "breathing-glow": return wrap(<div className="absolute" style={{ width: "80%", height: "60%", left: "10%", top: "20%", borderRadius: "50%", background: `radial-gradient(circle, ${a}80, transparent 70%)`, animation: "mvp-breathe 4s ease-in-out infinite" }} />);
      case "conic-spotlight": return wrap(<div className="absolute" style={{ width: 280, height: 280, left: "calc(50% - 140px)", top: "calc(40% - 140px)", background: `conic-gradient(from 0deg, ${a}80, transparent 25%, transparent 50%, ${a2}70, transparent 75%)`, borderRadius: "50%", animation: "mvp-spin 12s linear infinite" }} />);
      case "rising-particles": return wrap(<>{[0,1,2,3,4,5,6,7].map(i => <div key={i} className="absolute rounded-full" style={{ width: 8 - (i % 3), height: 8 - (i % 3), left: `${8 + i * 12}%`, bottom: 0, background: i % 2 === 0 ? a : a2, opacity: 0.9, boxShadow: `0 0 6px ${i % 2 === 0 ? a : a2}`, animation: `mvp-rise ${4 + i * 0.8}s ease-in-out infinite`, animationDelay: `${i * -0.6}s` }} />)}</>);
      ...
      case "floating-orbs": return wrap(<>{[{x:10,y:8,s:140},{x:65,y:45,s:120},{x:30,y:70,s:100}].map((p,i) => <div key={i} className="absolute rounded-full" style={{ width: p.s, height: p.s, left: `${p.x}%`, top: `${p.y}%`, background: `radial-gradient(circle, ${i % 2 === 0 ? a : a2}70, transparent 70%)`, filter: "blur(5px)", animation: `mvp-drift ${8 + i * 4}s ease-in-out infinite`, animationDelay: `${i * -3}s` }} />)}</>);
      case "layered-waves": return wrap(<>{[0,1,2,3,4].map(i => <div key={i} className="absolute w-[300%] left-[-100%]" style={{ height: `${50 + i * 18}px`, bottom: `${i * 16}px`, background: `linear-gradient(180deg, transparent 10%, ${i % 2 === 0 ? a : a2}80)`, borderRadius: "45% 45% 0 0", animation: `mvp-sway ${4 + i}s ease-in-out infinite`, animationDelay: `${i * -1}s` }} />)}</>);
      case "pulse-circles": return wrap(<>{[0,1,2,3].map(i => <div key={i} className="absolute rounded-full" style={{ width: 80, height: 80, left: "calc(50% - 40px)", top: "calc(45% - 40px)", border: `2px solid ${a}`, opacity: 0.6, animation: "mvp-ripple 4s ease-out infinite", animationDelay: `${i * 1}s` }} />)}</>);
      ...
      case "vortex-spin": return wrap(<div className="absolute" style={{ width: 300, height: 300, left: "calc(50% - 150px)", top: "calc(40% - 150px)", background: `conic-gradient(from 0deg, ${a}80, ${a2}60, transparent 40%, transparent 60%, ${a}70, ${a2}50, transparent)`, borderRadius: "50%", filter: "blur(6px)", animation: "mvp-spin 10s linear infinite" }} />);
      case "liquid-glass": return wrap(<>{[{x:25,y:25,s:150},{x:70,y:60,s:130}].map((p,i) => <div key={i} className="absolute" style={{ width: p.s, height: p.s, left: `${p.x}%`, top: `${p.y}%`, transform: "translate(-50%,-50%)", background: `radial-gradient(circle, ${i === 0 ? a : a2}60, transparent 65%)`, borderRadius: "60% 40% 30% 70% / 60% 30% 70% 40%", animation: "mvp-morph 7s ease-in-out infinite", animationDelay: `${i * -3.5}s`, filter: "blur(3px)" }} />)}</>);
      default: return null;
    }
  })();
