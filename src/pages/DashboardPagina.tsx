        ...
        <div className="flex-1 overflow-y-auto relative z-[2] scrollbar-none" style={{ fontFamily: `'${pFontB}', sans-serif` }}>
          ...
          <div className={`p-5 relative z-10 ${d.coverImageUrl ? "-mt-6" : ""}`}>
            ...
            <div className="flex flex-col items-center mb-3">
              ...
              <p className="font-extrabold text-[20px] leading-tight" style={{ color: d.nameColor || pText, fontFamily: `'${pFontH}', sans-serif`, textShadow: pTxtShadow, letterSpacing: "-0.025em" }}>
                {config.displayName || "Seu Nome"}
                {(config as any).verified && (
            ...
            <div className="flex flex-col items-center mb-3">
              ...
              {!templateAvatarConfig?.splitLayout && !templateAvatarConfig?.fullWidth && (() => {
                ...
                const showcaseStyle = (d as any).showcaseSocialStyle as "brand" | "mono" | undefined;
                const monoColor = d.nameColor || pText;
                const iconStyle = showcaseStyle === "mono" ? "template-mono"
                  : showcaseStyle === "brand" ? "brand"
                  : (d.socialIconStyle || "brand");
                ...
                iconsToRender.forEach((item: any, idx: number) => {
                  ...
                  const clr = iconStyle === "template-mono" ? monoColor
                    : iconStyle === "custom" ? customClr
                    : iconStyle === "theme" ? pText
                    : (bc || pText);
                  const bg = iconStyle === "template-mono" ? (pBg.startsWith("#f") ? "rgba(15,23,42,0.04)" : "rgba(255,255,255,0.08)")
                    : iconStyle === "custom" ? `${customClr}15`
                    : iconStyle === "theme" ? `${pAccent}15`
                    : (bc ? `${bc}18` : `${pAccent}15`);
                  const bdr = iconStyle === "template-mono" ? `1px solid ${monoColor}20`
                    : iconStyle === "custom" ? `1px solid ${customClr}25`
                    : iconStyle === "theme" ? `1px solid ${pAccent}18`
                    : (bc ? `1px solid ${bc}25` : `1px solid ${pAccent}18`);
                  const glassShadow = headerType
                    ? (iconStyle === "template-mono"
                      ? `inset 0 1px 0 ${pBg.startsWith("#f") ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.15)"}, inset 0 0 6px #4A8DFF15, 0 2px 4px rgba(0,0,0,0.08)`
                      : `inset 0 1px 0 rgba(255,255,255,0.2), inset 0 0 8px #4A8DFF15, 0 2px 4px rgba(0,0,0,0.10)`)
                    : "none";
                  icons.push(
                ...
                if (config.whatsapp && !placeholderSocials) {
                  const waClr = iconStyle === "template-mono" ? monoColor
                    : iconStyle === "custom" ? customClr
                    : iconStyle === "theme" ? pText
                    : "#25d366";
                  const waBg = iconStyle === "template-mono" ? (pBg.startsWith("#f") ? "rgba(15,23,42,0.04)" : "rgba(255,255,255,0.08)")
                    : iconStyle === "custom" ? `${customClr}15`
                    : iconStyle === "theme" ? `${pAccent}15`
                    : "rgba(37,211,102,0.15)";
                  const waBdr = iconStyle === "template-mono" ? `1px solid ${monoColor}20`
                    : iconStyle === "custom" ? `1px solid ${customClr}25`
                    : iconStyle === "theme" ? `1px solid ${pAccent}18`
                    : "1px solid rgba(37,211,102,0.25)";
                  const glassShadow = headerType ? `inset 0 1px 0 ${pBg.startsWith("#f") ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.15)"}, inset 0 0 6px #4A8DFF15, 0 2px 4px rgba(0,0,0,0.08)` : "none";
                  icons.push(
                    <div key="wa" className={`${btnSize} rounded-full flex items-center justify-center`} style={{ background: waBg, border: waBdr, boxShadow: glassShadow }}>
                ...
                if (!placeholderSocials) {
                  const shareClr = iconStyle === "template-mono" ? monoColor : pText;
                  const shareBg = iconStyle === "template-mono" ? (pBg.startsWith("#f") ? "rgba(15,23,42,0.04)" : "rgba(255,255,255,0.08)") : `${pAccent}15`;
                  const shareBdr = iconStyle === "template-mono" ? `1px solid ${monoColor}20` : `1px solid ${pAccent}18`;
                  const glassShadow = headerType ? `inset 0 1px 0 ${pBg.startsWith("#f") ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.15)"}, inset 0 0 6px #4A8DFF15, 0 2px 4px rgba(0,0,0,0.08)` : "none";
                  icons.push(
                    <div key="share" className={`${btnSize} rounded-full flex items-center justify-center`} style={{ background: shareBg, border: shareBdr, boxShadow: glassShadow }}>
                      <Share2 size={iconSize} style={{ color: shareClr }} />
                    </div>
                  );
                }
              ...
              if (block.type === "product") {
                ...
                return (
                  <div key={block.id} className="flex items-center gap-2.5 p-2.5 mb-2 transition-all hover:scale-[1.01]"
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold truncate" style={{ color: d.productTitleColor || pText, fontFamily: `'${pFontH}', sans-serif` }}>{p.title}</p>
                      <div className="flex items-center gap-2">
                        {p.originalPrice && (
                          <span className="text-[9px] line-through" style={{ color: d.originalPriceColor || pSub }}>{p.originalPrice}</span>
                        )}
                        {p.price && <p className="text-[10px] font-bold" style={{ color: d.priceColor || pAccent }}>{p.price}</p>}
                      </div>
                    </div>
                    {p.badge && (
